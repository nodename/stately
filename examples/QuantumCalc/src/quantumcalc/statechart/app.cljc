...
(defonce app
         {:states      {:app/on {:components [:calc :alert]}}
          ...
          :actions     {:app/clear-inputs-action (fn
                                                   [db]
...
(defonce calc
         {:states      {:calc/operand1   {:components [:operand1]}
          ...
          :transitions {[:calc/operand1 :operator-entered]   {:target  :calc/op-entered
                        ...
                        [:calc/op-entered :operator-entered] {:condition (fn [db [value]] (= value '-))
                                                              :target    :operand2/int}
...
(defonce operandX
         {:states      {:operandX/zero {:entry-actions [[:operandX/update-value-action]]}
                        :operandX/int  {:entry-actions [[:operandX/update-value-action]]}
                        :operandX/frac {:entry-actions [[:operandX/update-value-action]]}}

          :transitions {[:operandX/zero :zero-entered]  {:target :internal}
          ...
          :actions     {:operandX/update-value-action [append-value :operandX/value]
